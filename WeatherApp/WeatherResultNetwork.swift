...
extension WeatherResult: ImmutableMappable {
    ...
    public init(map: Map) throws {
        ...
        // Weather Icon
        if let weatherObjects = map.JSON["weather"] as! NSArray?,
           let weather = weatherObjects[0] as? [String:Any],
           let iconDescription = weather["icon"] {
            weatherIcon = iconDescription as! String
        } else {
            weatherIcon = ""
        }
        ...
        if let cityName = map.JSON["name"] {
            city = City(name: cityName as! String, timeRequested: Date())
        } else {
            city = City(name: "", timeRequested: Date())
        }

        // Location
        if let coord = map.JSON["coord"] as! [String:Any]?,
            let coord_lat = coord["lat"],
            let coord_lon = coord["lon"] {
            location = Location(lat: coord_lat as! Double, lon: coord_lon as! Double)
        } else {
